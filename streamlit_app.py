...
def process_excel_file_and_style(file_bytes_io):
    ...
    for sheet_name_raw in wb_raw.sheetnames:
        ...
        # --- NAType 오류 방어를 위한 컬럼명 사전 처리 (강력한 버전) ---
        # values[0] (첫 행)의 각 값을 순회하며 None 또는 Pandas NaN/NaT를 빈 문자열로 변환하고 strip 적용
        # 이렇게 하면 df.iloc[0]을 사용하기 전에 컬럼명 후보들이 모두 안전한 문자열이 됨
        safe_header_values = [
            str(cell_value).strip() if cell_value is not None and not pd.isna(cell_value) else ""
            for cell_value in values[0]
        ]
        
        # 원본 values[0]을 안전하게 처리된 헤더 값으로 교체
        values[0] = tuple(safe_header_values) # tuple로 다시 할당

        df = pd.DataFrame(values)
        # 이제 df.iloc[0]은 안전한 문자열 컬럼명만 포함할 것으로 예상됨
        ...
        for col in df.columns:
            # 컬럼이 존재하고, 해당 컬럼의 데이터 타입이 'object'(문자열)인 경우에만 처리
