...
def is_valid_email(email):
    """
    주어진 문자열이 기본적인 이메일 형식에 맞는지 검사합니다.
    (예: user@example.com)
    """
...
def process_excel_file_and_style(file_bytes_io):
    """
    엑셀 파일을 처리하고, 데이터를 정제하여 스타일을 적용한 후 BytesIO 객체로 반환합니다.
    """
...
# 사용자 아이디 입력 필드
user_id = st.text_input("아이디를 입력하세요 (예시:example@gmail.com, 관리자는 'admin' 입력)")

# --- 이메일 유효성 검사 및 'admin' 예외 처리 ---
if user_id: # user_id가 비어있지 않을 때만 검사
    # 'admin'일 경우 이메일 유효성 검사를 건너뛴다
    if user_id != "admin":
