...
def is_valid_email(email):
    ...
    return re.match(email_regex, email) is not None # 'email' 인자를 전달하도록 수정
...
def process_sheet_v8(df, professors_list, sheet_key):
    ...
    # pd.Series([" "] * len(df.columns), index=df.columns) 대신 pd.NA를 사용하여 Excel에서 빈 셀로 인식되도록 함
    empty_row_series = pd.Series([pd.NA] * len(df.columns), index=df.columns)
    # 첫 번째 컬럼에만 빈 문자열을 넣어 엑셀에서 빈 셀로 인식되도록 함 (선택사항, 필요 없으면 제거)
    if not empty_row_series.empty: # 빈 df일 경우를 대비하여 체크
        empty_row_series.iloc[0] = "" # 또는 pd.NA
    ...
    if not professor_header_row.empty: # 빈 df일 경우를 대비하여 체크
    ...
    # df.columns에 required_cols의 컬럼이 존재하는 경우에만 선택
...
def process_excel_file_and_style(file_bytes_io):
    ...
    for sheet_name_raw in wb_raw.sheetnames:
        ...
        # --- NAType 오류 방어를 위한 컬럼명 사전 처리 ---
        # 첫 행의 값들을 안전하게 문자열로 변환하고 공백 제거 (NaN/None을 빈 문자열로 처리)
        # 이 부분이 df.iloc[0]이 컬럼명으로 할당되기 전에 미리 'strip'을 안전하게 수행하는 역할
        processed_header = [str(col).strip() if col is not None and not pd.isna(col) else "" for col in values[0]]
        
        # 임시 DataFrame 생성 시 processed_header를 컬럼으로 사용
        df = pd.DataFrame(values[1:], columns=processed_header) # 첫 행은 헤더로 사용했으므로 values[1:]부터 데이터
        
        # 기존 코드에서 df.columns = df.iloc[0]과 df.drop([0])는 위에서 이미 처리되었으므로 필요 없음.
        # 따라서 이 부분은 삭제되거나 주석 처리됩니다. (주석 처리하여 원형 유지 요청에 부합)
        # df.columns = df.iloc[0] # << 이 라인이 NAType 오류의 주 원인. 위에서 이미 처리했으므로 제거
        # df = df.drop([0]).reset_index(drop=True) # << 이 라인도 마찬가지로 제거

        df = df.fillna("").astype(str) # NaN 값 채우고 모든 컬럼을 문자열로

        # '='로 시작하는 문자열 앞에 ''' 추가
        for col in df.columns:
            if df[col].dtype == 'object': # 문자열 (object) 타입 컬럼에만 적용
    ...
    for sheet_name in wb_styled.sheetnames:
        ...
        if ws.max_row > 1: # 헤더만 있는 경우는 건너뜀
