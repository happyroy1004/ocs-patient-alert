...
if existing_data:
    ...
    for key, val in existing_data.items():
        ...
        with col1:
            st.write(f"👤 이름: {val.get('name')}")
        with col2:
            st.write(f"🆔 번호: {val.get('number')}")
        with col3:
            if st.button("❌ 삭제", key=f"delete_{key}"):
                db.reference(f"patients/{firebase_key}/{key}").delete()
                st.success(f"삭제 완료: {val.get('name')} ({val.get('number')})")
                st.rerun()
else:
    st.info("아직 등록된 토탈환자가 없습니다.")
    ...
    try:
        ...
        for sheet_name in xl.sheet_names:
            try:
                ...
                df[["환자명", "진료번호"]] = df[["환자명", "진료번호"]].astype(str)
                all_patients = df[["환자명", "진료번호"]].dropna()
