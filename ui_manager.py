...
# professor_reviews_module 임포트 예외 처리 (파일 누락 시 대비)
try:
...
def show_login_and_registration():
    ...
    if mode == 'not_logged_in':
        ...
        num = st.text_input("원내생 번호 (예: 12)", key="reg_n") # 번호 필드
        ...
        if st.button("등록 완료"):
            if is_valid_email(email) and pw:
                ...
                st.session_state.update({'current_firebase_key': key, 'found_user_email': email, 'login_mode': 'user_mode'})
                st.rerun()

    elif mode == 'new_doctor_registration':
        st.subheader("🧑‍⚕️ 치과의사 신규 등록")
        name = st.text_input("이름", key="dr_n")
        num = st.text_input("식별 번호", key="dr_num") # 번호 필드
...
def show_admin_mode_ui():
    ...
    with tab_user_mgmt:
        if not st.session_state.admin_password_correct:
            ...
            if st.button("관리자 인증"):
                if admin_pw == st.secrets.get("admin", {}).get("password", DEFAULT_PASSWORD):
                    st.session_state.admin_password_correct = True; st.rerun()
                else: st.error("비밀번호가 틀렸습니다.")
        else:
            st.subheader("사용자 목록")
            # 사용자 관리 로직 (목록 출력 및 삭제 버튼)
...
def show_user_mode_ui(firebase_key, user_name):
    ...
    get_google_calendar_service(firebase_key) # 인증 객체 로드
    
    t1, t2, t3 = st.tabs(['환자 관리', 'OCS 분석', '리뷰'])
    with t1:
        st.subheader("환자 정보 등록")
        # 환자 등록 로직...
