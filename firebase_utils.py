...
# Google 인증 관련 라이브러리 (과거 코드 기반)
from google_auth_oauthlib.flow import Flow
from google.auth.transport.requests import Request
from googleapiclient.discovery import build

# 사용할 권한 범위 (과거 코드 [cite: 20] 참고)
...
# --- 3. 자격 증명 저장/로드 (과거 방식 복원 [cite: 4, 5, 6]) ---
def save_google_creds_to_firebase(user_id_safe, creds):
    try:
        # 과거 코드 구조인 users/{user_id_safe}/google_creds 경로 사용 
...
# --- 4. Google 캘린더 서비스 관리 (과거 세션 관리 복원 [cite: 21, 23, 24]) ---
def get_google_calendar_service(user_id_safe):
    ...
    # 이미 유효한 토큰이 있는 경우 서비스 반환 [cite: 26]
    if creds:
    ...
    # 인증 후 리다이렉트되어 돌아온 경우 처리 [cite: 23, 24]
    ...
    # 인증 흐름 생성 (Missing code verifier 방지용 세션 고정) [cite: 22, 23]
